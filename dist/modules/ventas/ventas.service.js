...
exports.deleteVenta = exports.updateVenta = exports.reporteGananciaBarbero = exports.reportePorFecha = exports.getVentaById = exports.crearVenta = exports.getVentas = void 0;
...
const getVentas = async () => {
  ...
  `);
    ...
    for (const venta of ventas) {
      ...
      `);
        const serviciosResult = await database_1.pool.request()
            .input("id_ventas", venta.id_ventas)
            .query(`
      ...
      `);
        ...
        if (data.productos) {
            ...
            for (const item of data.productos) {
                ...
                // Validar stock
                const stockRequest = new mssql_1.default.Request(transaction);
          ...
          `);
                ...
                const idDetalleProdRequest = new mssql_1.default.Request(transaction);
        ...
        `);
                ...
                const insertRequest = new mssql_1.default.Request(transaction);
                await insertRequest
          ...
          `);
                ...
                // Descontar stock
                const updateRequest = new mssql_1.default.Request(transaction);
                await updateRequest
                    .input("id_producto", item.id_producto)
                    .input("cantidad", item.cantidad)
                    .query(`
        ...
        if (data.servicios) {
            for (const item of data.servicios) {
                ...
                const serviceRequest = new mssql_1.default.Request(transaction);
...
const updateVenta = async (id, data) => {
    ...
    const ventaExists = await database_1.pool.request()
        .input('id_ventas', id)
    ...
    if (data.id_estado !== undefined && data.id_estado !== null) {
        const estadoExists = await database_1.pool.request()
            .input('id_estado', data.id_estado)
        ...
        if (data.id_estado === 3) {
            ...
            const productosVenta = await database_1.pool.request()
                .input('id_ventas', id)
                .query(`
    ...
    if (data.id_cliente !== undefined && data.id_cliente !== null) {
        const clienteExists = await database_1.pool.request()
            .input('id_cliente', data.id_cliente)
    ...
    if (data.id_barbero !== undefined && data.id_barbero !== null) {
        const barberoExists = await database_1.pool.request()
            .input('id_barbero', data.id_barbero)
