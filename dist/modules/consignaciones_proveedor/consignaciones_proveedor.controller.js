...
const service = __importStar(require("./consignaciones_proveedor.service"));
const zod_1 = require("zod");
const consignaciones_proveedor_schema_1 = require("./consignaciones_proveedor.schema");
...
const obtenerPorId = async (req, res) => {
    try {
        ...
        const { id: validatedId } = consignaciones_proveedor_schema_1.consignacionIdSchema.parse({ id });
        const consignacion = await service.getConsignacionById(validatedId);
        if (!consignacion)
            return res.status(404).json({ success: false, message: "Consignacion no encontrada" });
...
const crear = async (req, res) => {
    try {
        ...
        const validatedData = consignaciones_proveedor_schema_1.createConsignacionSchema.parse(req.body);
...
const actualizar = async (req, res) => {
    try {
        ...
        const validatedData = consignaciones_proveedor_schema_1.updateConsignacionSchema.parse(req.body);
