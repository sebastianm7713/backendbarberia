...
export const crearCita = async (data: any) => {
  ...
  const barberoResult = await pool.request()
...
export const listarCitas = async () => {
  ...
  const citas = result.recordset.map((cita: any) => {
...
export const cambiarEstado = async (id: number, estado: string) => {
  ...
  try {
    ...
    if (estado === "confirmada") {
      ...
      let reqServicio = new sql.Request(transaction);
      ...
      const normalizeTime = (timeValue: any) => {
      ...
      let reqUpdateExecution = new sql.Request(transaction);
      ...
      let reqUpdateComplete = new sql.Request(transaction);
      ...
      let reqUpdateOther = new sql.Request(transaction);
    ...
    if (estado === "completado") {
      ...
      let req1 = new sql.Request(transaction);
      ...
      let req2 = new sql.Request(transaction);
      ...
      let req3 = new sql.Request(transaction);
      await req3
      ...
      let req4 = new sql.Request(transaction);
      await req4
        .input("id_ventas", id_ventas)
        .input("id_cita", id)
        .query(`
      ...
      let req5 = new sql.Request(transaction);
      ...
      let req6 = new sql.Request(transaction);
      await req6
      ...
      for (const producto of productosCita) {
        ...
        let reqProducto = new sql.Request(transaction);
        ...
        const verifyInsert = await new sql.Request(transaction)
          .input("id_detalle_producto", id_detalle_producto)
          .query(`
      ...
      const reqCountProductos = new sql.Request(transaction);
...
export const actualizarCita = async (id: number, data: any) => {
  ...
  if (data.id_barbero !== undefined) updateData.id_barbero = data.id_barbero;
  if (data.id_servicio !== undefined) updateData.id_servicio = data.id_servicio;
  ...
  if (stateLogicRequired) {
    if (Object.keys(updateData).length > 0) {
      ...
      const updateRequest = pool.request().input("id_cita", id);
    ...
    return await cambiarEstado(id, data.estado);
  }
...
export const obtenerHorasDisponibles = async (id_barbero: number, fecha: string) => {
  ...
  const horasOcupadas = citasResult.recordset.map((cita: any) => cita.hora);
