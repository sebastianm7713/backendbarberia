...
function convertEstadoToBit(estado: any): number | null {
  if (estado === 'activo' || estado === 1 || estado === true || estado === '1' || estado === 'true') return 1;
  if (estado === 'inactivo' || estado === 0 || estado === false || estado === '0' || estado === 'false') return 0;
  return null;
}

const convertBitToEstado = (bit: boolean): string => {
...
export const updateUsuario = async (id: number, data: any) => {
  ...
  if (updates.length === 0) return await getUsuarioById(id);
